...
public:
    ...
    inline vecN operator/(const vecN& that) const
    {
        ...
        int n;
        for (n = 0; n < len; n++)
            result.data[n] = data[n] * that.data[n]; // TODO: Issue, is it wrong?
        return result;
    }
...
public:
    ...
    inline const T x() const { return base::data[0]; }
    inline const T y() const { return base::data[1]; }
    inline const T z() const { return base::data[2]; }
};
...
public:
    ...
    inline const T y() const { return base::data[1]; }
    inline const T z() const { return base::data[2]; }
    inline const T w() const { return base::data[3]; }

    inline const Tvec3<T> xyz() const { return Tvec3<T>(x(), y(), z()); }
};
...
public:
    ...
    template<const int tw, const int th>
    inline my_type operator*(const matNM<T, tw, th>& that) const
    {
        ensure<w == th>();

        matNM<T, tw, h> result(0);
